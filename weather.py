...
logging.basicConfig(level=logging.INFO, filename='crash.log', format='%(asctime)s :: %(levelname)s :: Module %(module)s :: Line No %(lineno)s :: %(message)s' )
...
def get_city() -> str:
    try:
        ...
        logging.exception("Exception occured in get_city.")
        quit()
...
class WeatherData():
    ...
    _location: str | None = None # Current location
    ...
    def __init__(self, day_offset: int, location: str) -> None: # Void function
        ...
        if validation_check == 2:
            ...
            try:
                ...
                logging.exception("Exception occured in WeatherData.__init__.")
